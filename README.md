...
- **`/kiro-getspecs` for brownfield bootstrap.** Existing codebase with no `.kiro/` specs? Reverse-engineer steering, roadmap, and spec seeds from code, then continue with `/kiro-spec-requirements` or `/kiro-spec-batch`. See [Brownfield guide](docs/guides/brownfield-getspecs.md).
