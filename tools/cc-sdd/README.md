...
- **`/kiro-getspecs` for brownfield bootstrap.** Existing codebase with no `.kiro/` specs? Reverse-engineer steering, roadmap, and spec seeds (`brief.md` + `spec.json`) from code — then continue with `/kiro-spec-requirements` or `/kiro-spec-batch`. See [Brownfield guide](https://github.com/gotalab/cc-sdd/blob/main/docs/guides/brownfield-getspecs.md).
