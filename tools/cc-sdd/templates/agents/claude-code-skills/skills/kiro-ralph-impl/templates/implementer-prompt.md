...
> If >70%: return PARTIAL_COMPLETION immediately (see Status Report format). If 60–70%: warn `[CTX: X% — wrapping up]` and continue only to a clean stopping point. If `unavailable`: log warning and continue normally.
...
> **Context check** (after building Task Brief): Run the `Context check command` verbatim.
...
> **Context check** (after RED and GREEN phases): Run the `Context check command` verbatim.
...
> **Context check** (after VERIFY): Run the `Context check command` verbatim.
