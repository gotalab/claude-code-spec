...
const manifestPath = join(repoRoot, 'tools/cc-sdd/templates/manifests/antigravity-skills.json');
...
describe('real antigravity-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecQuick = join(cwd, '.agent/skills/kiro-spec-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.agent/skills/kiro-spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.agent/skills/kiro-validate-impl/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.agent/skills/kiro-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.agent/skills/kiro-spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.agent/skills/kiro-spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.agent/skills/kiro-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/kiro-spec-requirements/rules/ears-format.md'))).toBe(true);
    ...
    const skillSpecTasks = join(cwd, '.agent/skills/kiro-spec-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.agent/skills/kiro-spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.agent/skills/kiro-spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.agent/skills/kiro-spec-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 18 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.agent/skills/kiro-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.agent/skills/kiro-impl/templates/reviewer-prompt.md');
