...
const manifestPath = join(repoRoot, 'tools/cc-sdd/templates/manifests/windsurf-skills.json');
...
describe('real windsurf-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecQuick = join(cwd, '.windsurf/skills/kiro-spec-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.windsurf/skills/kiro-spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.windsurf/skills/kiro-validate-impl/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.windsurf/skills/kiro-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.windsurf/skills/kiro-spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.windsurf/skills/kiro-spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.windsurf/skills/kiro-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-spec-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-spec-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-getspecs/rules/getspecs-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-spec-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/kiro-spec-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.windsurf/skills/kiro-spec-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.windsurf/skills/kiro-spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.windsurf/skills/kiro-spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.windsurf/skills/kiro-spec-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 18 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.windsurf/skills/kiro-impl/templates/implementer-prompt.md');
